...
.PHONY: all
all: fmt lint vet $(BIN) ; $(info $(M) building executable…) @ ## Build program binary
	$Q $(GO) build \
		-tags release \
		-ldflags '-X main.Version=$(VERSION) -X main.BuildDate=$(DATE)' \
		-o $(BIN)/$(PACKAGE) main.go version.go ec2instance.go filesystem.go upload.go verify.go
...
test-xml: fmt lint vet | $(GO2XUNIT) ; $(info $(M) running $(NAME:%=% )tests…) @ ## Run tests with xUnit output
	$Q mkdir -p test
...
.PHONY: vet
vet: ; $(info $(M) running go tool vet -shadow…) @ ## Vet for shadow variables
	$Q $(GO) tool vet \
		-shadow .
